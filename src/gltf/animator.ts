...
import type {ActiveAnimation} from './animation.js';
import {
  KeyFrame,
  Model,
  Skin,
    ...
    activeAnimations[track].forEach(rootAnimation => {
      ...
      Object.keys(model.animations[rootAnimation.key]).forEach(key => {
        ...
        localTransform = localTransform
          .multiplyMatrix(Matrix4.translate(transform.t.get(0), transform.t.get(1), transform.t.get(2)))
          .multiplyMatrix(rotTransform)
          .multiplyMatrix(Matrix4.scale(transform.s.get(0), transform.s.get(1), transform.s.get(2)));
